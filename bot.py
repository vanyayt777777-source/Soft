...
class Database:
    ...
    async def get_referral_earnings(self, referrer_id: int, bot_token: str) -> float:
        async with self.pool.acquire() as conn:
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if len(args) > 1:
        try:
            # Проверяем, является ли аргумент ID реферера или токеном бота
            if args[1].startswith("bot_"):
                bot_token = args[1][4:]  # Убираем префикс "bot_"
            else:
    ...
    await db.create_user(user_id, username, referrer_id, bot_token)
    
    # Проверяем, является ли бот реферальным
    is_referral_bot = bot_token is not None
    
    # Для основного бота проверяем подписку
    if not is_referral_bot and bot_token is None:
...
async def referral_menu(message: Message):
    ...
    async with db.pool.acquire() as conn:
        rows = await conn.fetch("SELECT earnings FROM referrals WHERE referrer_id = $1", user_id)
        for row in rows:
...
async def process_bet_amount(message: Message, state: FSMContext):
    try:
        ...
        # Получаем информацию о пользователе (для проверки реферального бота)
        user = await db.get_user(user_id)
        bot_token = user.get("bot_token") if user else None
        ...
        if win:
            ...
            # Отчисление рефереру (10% от проигрыша) - только если это не реферальный бот
            if not bot_token and user and user.get("referrer_id"):
            ...
            # Отчисление владельцу реферального бота (10% от проигрыша)
            if bot_token:
                async with db.pool.acquire() as conn:
...
async def run_referral_bot(bot_token: str):
    ...
    @referral_dp.message(BetStates.waiting_for_bet_amount)
    ...
    @referral_dp.message(BetStates.waiting_for_deposit_amount)
    ...
    @referral_dp.message(BetStates.waiting_for_withdraw_amount)
    ...
    asyncio.create_task(referral_dp.start_polling(referral_bot))
...
async def main():
    ...
    async with db.pool.acquire() as conn:
        ...
        for bot_record in bots:
            ...
            logger.info(f"Запущен реферальный бот с токеном {bot_record['token'][:10]}...")
