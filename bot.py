    ...
    async def get_total_stats(self) -> Dict:
        ...
        async with self.pool.acquire() as conn:
            ...
            total_withdrawn = await conn.fetchval(
                "SELECT COALESCE(SUM(amount), 0) FROM withdrawals WHERE status = 'created'"
            )
...
class AnonDonateBot:
    ...
    async def links_handler(self, message: Message):
        ...
        text = (
            ...
            f"{hcode(f't.me/{self.bot_username}?start=msg_{msg_link["code"]}')}\n\n"
            f"{hbold('💰 Для донатов:')}\n"
            f"{hcode(f't.me/{self.bot_username}?start=donate_{donate_link["code"]}')}"
        )
    ...
    async def copy_link(self, callback: CallbackQuery, link_type: str):
        ...
        await callback.message.answer(
            f"{hcode(f't.me/{self.bot_username}?start={link_type}_{code}')}"
        )
    ...
    async def regenerate_links(self, callback: CallbackQuery):
        ...
        text = (
            ...
            f"{hbold('📝 Для сообщений:')}\n"
            f"{hcode(f't.me/{self.bot_username}?start=msg_{msg_code}')}\n\n"
            f"{hbold('💰 Для донатов:')}\n"
            f"{hcode(f't.me/{self.bot_username}?start=donate_{donate_code}')}"
        )
    ...
    async def process_reply(self, message: Message, state: FSMContext):
        ...
        await self.bot.send_message(
            ...
            f'"{message.text}"',
            reply_markup=self.keyboards.message_actions(msg_row)
        )
                ...
                if invoices and isinstance(invoices, list) and len(invoices) > 0:
                    ...
                    if invoice.get('status') == 'paid':
                        ...
                        await self.bot.send_message(
                            ...
                            reply_markup=self.keyboards.donation_actions(don_row)
                        )
    ...
    async def start_reply(self, callback: CallbackQuery):
        ...
        # Не устанавливаем состояние, так как ответ уже обрабатывается в process_reply
        # Но нужно различать ответ на сообщение и новое сообщение
        # Временно сохраняем флаг в state
        await state.update_data(is_reply=True)
        await state.set_state(ReplyStates.waiting_for_reply)
