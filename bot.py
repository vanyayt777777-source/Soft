...
class Database:
    ...
    async def get_user_by_token(self, token: str) -> Optional[Dict]:
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow("SELECT * FROM users WHERE auth_token = $1", token)
            return dict(row) if row else None
    ...
    async def get_referral_earnings(self, referrer_id: int, bot_token: str = None) -> Dict:
        async with self.pool.acquire() as conn:
            if bot_token:
    ...
    async def get_stats(self):
        async with self.pool.acquire() as conn:
            ...
            total_volume_usdt = await conn.fetchval("SELECT COALESCE(SUM(CASE WHEN currency='USDT' THEN bet_amount ELSE 0 END), 0) FROM bets")
            total_volume_ton = await conn.fetchval("SELECT COALESCE(SUM(CASE WHEN currency='TON' THEN bet_amount ELSE 0 END), 0) FROM bets")
    ...
    # Методы для игры Mines
    async def create_mines_game(self, user_id: int, game_id: str, bet_amount: float, currency: str, bombs_count: int):
        async with self.pool.acquire() as conn:
            # Создаем поле 5x5
            field = []
            for i in range(25):
                field.append("bomb" if i < bombs_count else "gem")
            random.shuffle(field)
            field_str = ",".join(field)
            
            await conn.execute("""
                INSERT INTO mines_games (user_id, game_id, bet_amount, currency, bombs_count, field, opened_cells, multiplier, is_active)
                VALUES ($1, $2, $3, $4, $5, $6, ARRAY[]::TEXT[], 1.0, TRUE)
            """, user_id, game_id, bet_amount, currency, bombs_count, field_str)
            
            return field
    
    async def get_mines_game(self, game_id: str):
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow("SELECT * FROM mines_games WHERE game_id = $1", game_id)
            return dict(row) if row else None
    
    async def update_mines_game(self, game_id: str, cell_index: int, new_multiplier: float):
        async with self.pool.acquire() as conn:
            await conn.execute("""
                UPDATE mines_games 
                SET opened_cells = array_append(opened_cells, $2), multiplier = $3
                WHERE game_id = $1
            """, game_id, str(cell_index), new_multiplier)
    
    async def end_mines_game(self, game_id: str, result: str):
        async with self.pool.acquire() as conn:
            await conn.execute("UPDATE mines_games SET is_active = FALSE WHERE game_id = $1", game_id)
            
            if result == "win":
                game = await self.get_mines_game(game_id)
                if game:
    ...
    async def get_crash_game(self, game_id: str):
        async with self.pool.acquire() as conn:
...
async def crash_menu(message: Message):
    ...
    await message.answer(
        ...
        f"• Лучший множитель: x{best_multiplier:.2f}",
...
async def crash_stats(callback: CallbackQuery):
    ...
    if stats:
        for stat in stats:
            ...
            text += f"• Средний множитель: x{stat['avg_multiplier']:.2f}\n\n"
    else:
...
async def process_crash_auto(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            f"💥 Крашнется на x{crash_point:.2f}",
            parse_mode="HTML"
        )
        
        # Запускаем игру
        asyncio.create_task(run_crash_game(message.chat.id, game_id, crash_point, auto_stop))
...
async def run_crash_game(chat_id: int, game_id: str, crash_point: float, auto_stop: float):
    ...
    while multiplier < crash_point and not crashed:
        ...
        if auto_stop > 0 and multiplier >= auto_stop:
            ...
            await msg.edit_text(
                ...
                f"💵 Выигрыш: {bet_amount * multiplier:.2f} {currency}",
                parse_mode="HTML"
            )
            return
...
async def crash_cashout(callback: CallbackQuery):
    ...
    # Забираем выигрыш
    success = await db.cashout_crash_game(game_id, game['multiplier'])
    if success:
        await callback.message.edit_text(
            f"💎 <b>Crash Game #{game_id}</b>\n\n"
            f"✅ <b>ВЫ ЗАБРАЛИ!</b>\n"
            f"💰 Множитель: x{game['multiplier']:.2f}\n"
            f"💵 Выигрыш: {game['bet_amount'] * game['multiplier']:.2f} {game['currency']}",
            parse_mode="HTML"
        )
...
async def profile_menu(message: Message):
    ...
    await message.answer(
        ...
        f"💎 Лучший выигрыш: {biggest_win:.2f}$\n"
...
async def show_history(callback: CallbackQuery):
    ...
    for bet in bets:
        ...
        text += f"{emoji} {time_str} | {result_emoji} | {bet['bet_amount']} {bet['currency']} → {bet['win_amount']} (x{bet['multiplier']})\n"
...
async def show_stats(callback: CallbackQuery):
    ...
    if stats:
        ...
        for stat in stats:
            ...
            win_rate = (stat['wins'] / stat['games'] * 100) if stat['games'] > 0 else 0
            profit = stat['total_won'] - stat['total_bet']
    ...
    if daily:
        ...
        for day in daily:
            text += f"📅 {day['date'].strftime('%d.%m')}: {day['games']} игр, {day['volume']:.2f}$\n"
...
async def top_players(message: Message):
    ...
    for i, player in enumerate(top, 1):
        ...
        text += f"   🎯 Побед: {player['total_wins']} | 📊 {win_rate:.1f}% | 💰 {player['total_balance']:.2f}$\n\n"
...
async def process_bonus_code(message: Message, state: FSMContext):
    ...
    if result is None:
        ...
        if result['usdt'] > 0:
            text += f"💰 +{result['usdt']} USDT зачислено!\n"
        if result['ton'] > 0:
            text += f"💰 +{result['ton']} TON зачислено!\n"
...
async def admin_bots_list(callback: CallbackQuery):
    ...
    for bot in bots:
        ...
        text += f"  💰 Заработано: {bot['total_earnings']:.2f}$\n"
...
async def process_new_balance(message: Message, state: FSMContext):
    ...
    try:
        ...
        if currency == 'USDT':
            await db.update_balance(user_id, amount_usdt=new_balance - current['usdt'])
        else:
            await db.update_balance(user_id, amount_ton=new_balance - current['ton'])
...
async def admin_create_bonus(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "<code>КОД USDT_СУММА TON_СУММА КОЛ-ВО ДНЕЙ</code>\n\n"
        "Например: <code>WELCOME 10 5 30</code>\n"
        "(код WELCOME на 10 USDT и 5 TON, 30 дней, без ограничений)",
        parse_mode="HTML"
    )
