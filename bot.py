...
class Database:
    ...
    async def create_crash_game(self, user_id: int, game_id: str, bet_amount: float, auto_stop: float):
        async with self.pool.acquire() as conn:
            crash_point = random.uniform(1.01, 10.0)
            await conn.execute("""
                INSERT INTO crash_games (user_id, game_id, bet_amount, auto_stop, crash_point, is_active)
                VALUES ($1, $2, $3, $4, $5, TRUE)
            """, user_id, game_id, bet_amount, auto_stop, crash_point)
            return crash_point
    ...
    async def cashout_crash_game(self, game_id: str, multiplier: float):
        async with self.pool.acquire() as conn:
            ...
            if game and game['is_active'] and not game['cashed_out']:
                ...
                await self.update_balance(game['user_id'], win_amount)
...
async def crash_play(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        "💎 <b>Игра Crash</b>\n\n"
        "Введите сумму ставки в USDT (минимум 0.01):",
        parse_mode="HTML"
    )
...
async def process_crash_bet(message: Message, state: FSMContext):
    try:
        ...
        if bet_amount < 0.01:
            await message.answer("❌ Минимальная ставка 0.01 USDT")
            return
...
async def process_crash_auto(message: Message, state: FSMContext):
    try:
        ...
        # Создаем игру
        crash_point = await db.create_crash_game(user_id, game_id, bet_amount, auto_stop)
        ...
        # Запускаем игру
        asyncio.create_task(run_crash_game(message.chat.id, game_id, crash_point, auto_stop, bet_amount))
...
async def run_crash_game(chat_id: int, game_id: str, crash_point: float, auto_stop: float, bet_amount: float):
    ...
    if game and game['is_active'] and not game['cashed_out']:
        ...
        await msg.edit_text(
            ...
            f"❌ Вы проиграли {bet_amount} USDT!",
            parse_mode="HTML"
        )
...
async def top_players(message: Message):
    ...
    for i, player in enumerate(top, 1):
        ...
        text += f"   🎯 Побед: {player['total_wins']} | 📊 {win_rate:.1f}% | 💰 {float(player['balance']):.2f}$\n\n"
...
async def run_referral_bot(bot_token: str):
    ...
    try:
        ...
        # Копируем обработчики Crash для реферального бота
        @referral_dp.callback_query(F.data == "crash_play")
        async def referral_crash_play(callback: CallbackQuery, state: FSMContext):
            await callback.message.edit_text(
                "💎 <b>Игра Crash</b>\n\n"
                "Введите сумму ставки в USDT (минимум 0.01):",
                parse_mode="HTML"
