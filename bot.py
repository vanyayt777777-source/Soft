...
Анонимные донаты - отправитель не раскрывается
"""
...
class Database:
    ...
    async def add_message(self, sender_id: Optional[int], receiver_id: int, text: str) -> int:
        """Добавить сообщение"""
        async with self.pool.acquire() as conn:
    ...
    async def add_donation(self, receiver_id: int, amount: Decimal, message: str = None, invoice_id: str = None):
        """Добавить донат (анонимно - без sender_id)"""
        async with self.pool.acquire() as conn:
            await conn.execute("""
                INSERT INTO donations (receiver_id, amount, message, invoice_id, status, paid_at)
                VALUES ($1, $2, $3, $4, 'completed', NOW())
            """, receiver_id, amount, message, invoice_id)
    ...
    async def create_withdrawal(self, user_id: int, amount: Decimal, check_id: str, check_url: str) -> int:
        """Создать вывод"""
        async with self.pool.acquire() as conn:
            ...
            await self.update_balance(user_id, Decimal(str(-amount)))
    ...
    def links_inline(user_id: int, msg_code: str, donate_code: str, bot_username: str) -> InlineKeyboardMarkup:
        buttons = [
            [
                ...
                InlineKeyboardButton(text="📤 Поделиться", url=f"https://t.me/share/url?url=https://t.me/{bot_username}?start=msg_{msg_code}")
            ],
            [
                InlineKeyboardButton(text="💰 Копировать донаты", callback_data=f"copy_donate_{donate_code}"),
                InlineKeyboardButton(text="📤 Поделиться", url=f"https://t.me/share/url?url=https://t.me/{bot_username}?start=donate_{donate_code}")
            ],
...
class AnonDonateBot:
    ...
    async def start(self):
        ...
        await self.bot.send_message(
            ADMIN_ID,
            f"<b>✅ Бот запущен!</b>\n\n"
            f"Время: {self.start_time.strftime('%d.%m.%Y %H:%M:%S')}"
        )
    ...
    async def cmd_start(self, message: Message):
        ...
        text = (
            ...
            f"Этот бот позволяет получать <b>анонимные сообщения и донаты</b>.\n\n"
            f"<b>📌 Как это работает:</b>\n"
            f"1️⃣ Создайте ссылки в разделе <code>🔗 Мои ссылки</code>\n"
            f"2️⃣ Поделитесь ими с друзьями\n"
            f"3️⃣ Получайте анонимные сообщения и донаты\n"
            f"4️⃣ Выводите средства в профиле\n\n"
            f"<i>Минимальная сумма вывода: {MIN_WITHDRAW} USDT</i>"
        )
        
        await message.answer(text, reply_markup=self.keyboards.main_menu())
    ...
    async def cmd_admin(self, message: Message):
        ...
        text = (
            ...
            f"💰 Донатов: {stats['total_donations']} USDT\n"
            f"💸 Выведено: {stats['total_withdrawn']} USDT\n"
            f"📝 Сообщений: {stats['messages_count']}\n\n"
            f"<b>⏱ Аптайм:</b> {uptime.days}д {hours}ч {minutes}м"
        )
        
        await message.answer(text, reply_markup=self.keyboards.admin_menu())
    ...
    async def profile_handler(self, message: Message):
        ...
        text = (
            f"<b>👤 Ваш профиль</b>\n\n"
            f"💰 <b>Баланс:</b> {user['balance']} USDT\n\n"
            f"<b>📊 Статистика:</b>\n"
        ...
        if stats["last_messages"]:
            ...
            for msg in stats["last_messages"]:
                ...
                text += f'• "<i>{msg_text}</i>" ({time})\n'
        
        if stats["last_donations"]:
            text += f"\n<b>🎁 Последние донаты:</b>\n"
            for don in stats["last_donations"]:
                time = self._format_time(don['created_at'])
                msg = f" <i>\"{don['message'][:20]}...\"</i>" if don.get('message') and len(don['message']) > 20 else f" <i>\"{don.get('message', '')}\"</i>" if don.get('message') else ""
                text += f"• +{don['amount']} USDT{msg} ({time})\n"
        
        await message.answer(text, reply_markup=self.keyboards.profile_inline(user_id))
    ...
    async def links_handler(self, message: Message):
        ...
        if not msg_link:
            ...
            msg_link = {"code": code, "clicks": 0}
        if not donate_link:
            code = await self.db.create_link(user_id, "donate")
            donate_link = {"code": code, "clicks": 0}
        
        # Создаем ссылки
        msg_url = f"https://t.me/{self.bot_username}?start=msg_{msg_link['code']}"
        donate_url = f"https://t.me/{self.bot_username}?start=donate_{donate_link['code']}"
        
        text = (
            f"<b>🔗 Ваши персональные ссылки</b>\n\n"
            f"<b>📝 Для анонимных сообщений:</b>\n"
            f"<code>{msg_url}</code>\n"
            f"👁 Переходов: {msg_link.get('clicks', 0)}\n\n"
            f"<b>💰 Для анонимных донатов:</b>\n"
            f"<code>{donate_url}</code>\n"
            f"👁 Переходов: {donate_link.get('clicks', 0)}\n\n"
            f"<i>Нажмите на ссылку чтобы скопировать или поделиться</i>"
        )
    ...
    async def help_handler(self, message: Message):
        ...
        text = (
            ...
            f"<b>📝 Анонимные сообщения:</b>\n"
            f"1. Создайте ссылку в разделе <code>🔗 Мои ссылки</code>\n"
            f"2. Отправьте её кому хотите\n"
            f"3. Когда напишут - получите уведомление\n"
            f"4. Можете ответить анонимно\n\n"
            f"<b>💰 Анонимные донаты:</b>\n"
            f"1. Используйте ссылку для донатов\n"
            f"2. Отправитель оплачивает через Crypto Bot\n"
            f"3. Средства зачисляются на ваш баланс\n"
            f"4. Отправитель остается анонимным\n\n"
            f"<b>💸 Вывод средств:</b>\n"
            f"1. Зайдите в <code>📋 Мой профиль</code>\n"
            f"2. Нажмите <code>💸 ВЫВЕСТИ</code>\n"
            f"3. Введите сумму\n"
            f"4. Получите чек и активируйте в @CryptoBot\n\n"
    ...
    async def start_withdraw(self, callback: CallbackQuery):
        ...
        await callback.message.edit_text(
            f"<b>💸 Вывод средств</b>\n\n"
            f"💰 Ваш баланс: <b>{user['balance']} USDT</b>\n"
            f"📉 Минимальная сумма: {MIN_WITHDRAW} USDT\n\n"
            f"<i>Введите сумму для вывода (например: 5.50):</i>",
    ...
    async def copy_link(self, callback: CallbackQuery, link_type: str):
        ...
        url = f"https://t.me/{self.bot_username}?start={link_type}_{code}"
    ...
    async def regenerate_links(self, callback: CallbackQuery):
        ...
        msg_url = f"https://t.me/{self.bot_username}?start=msg_{msg_code}"
        donate_url = f"https://t.me/{self.bot_username}?start=donate_{donate_code}"
        
        text = (
            f"<b>✅ Новые ссылки созданы!</b>\n\n"
            f"<b>📝 Для сообщений:</b>\n"
            f"<code>{msg_url}</code>\n\n"
            f"<b>💰 Для донатов:</b>\n"
            f"<code>{donate_url}</code>\n\n"
            f"<i>Старые ссылки больше не работают</i>"
        )
            ...
            async with self.crypto as crypto:
                try:
                    ...
                    await message.answer(
                        ...
                        f"<code>{check['check_url']}</code>\n\n"
                        f"<b>📌 Инструкция:</b>\n"
                        f"1. Перейдите по ссылке\n"
                        f"2. Откроется @CryptoBot\n"
                        f"3. Нажмите «Забрать»\n"
                        f"4. Средства зачислятся на ваш баланс\n\n"
                        f"💰 Новый баланс: {new_user['balance']} USDT"
                    )
                    
                except Exception as e:
                    logger.error(f"Ошибка создания чека: {e}")
                    await message.answer("❌ Ошибка при создании чека. Попробуйте позже.")
            
            await state.clear()
            
        except ValueError:
            await message.answer("❌ Введите число (например: 5.50)")
    ...
    async def handle_start_param(self, message: Message, command: CommandStart):
        ...
        logger.info(f"Получен параметр: {args}")
        
        # Обработка ссылок на сообщения
        if args.startswith("msg_"):
            code = args[4:]
            logger.info(f"Обработка сообщения с кодом: {code}")
            await self.process_message_link(message, code)
            return
        
        # Обработка ссылок на донаты
        if args.startswith("donate_"):
            code = args[7:]
            logger.info(f"Обработка доната с кодом: {code}")
    ...
    async def process_message_link(self, message: Message, code: str):
        ...
        logger.info(f"process_message_link вызван с code: {code}")
        
        receiver_id = await self.db.get_receiver_by_code(code)
        logger.info(f"Найден receiver_id: {receiver_id}")
        ...
        await message.answer(
            f"<b>📝 Напишите анонимное сообщение</b>\n\n"
            f"<i>Сообщение будет доставлено анонимно. Отправитель останется неизвестным.</i>",
            reply_markup=self.keyboards.cancel()
        )
    ...
    async def process_reply(self, message: Message, state: FSMContext):
        ...
        msg_id = await self.db.add_message(message.from_user.id, receiver_id, message.text)
        ...
        await self.bot.send_message(
            ...
            f'"{message.text}"\n\n'
            f"<i>Чтобы ответить, нажмите кнопку ниже</i>",
            reply_markup=self.keyboards.message_actions(msg_id)
        )
    ...
    async def process_donate_link(self, message: Message, code: str):
        """Обработка ссылки на донат (полностью анонимно)"""
        logger.info(f"process_donate_link вызван с code: {code}")
        ...
        await message.answer(
            f"<b>💰 Отправка анонимного доната</b>\n\n"
            f"Минимальная сумма: {MIN_WITHDRAW} USDT\n\n"
            f"<i>Ваш донат будет полностью анонимным. Получатель не узнает, кто отправил.</i>\n\n"
            f"Введите сумму в USDT (например: 5.50):",
            reply_markup=self.keyboards.cancel()
        )
            ...
            async with self.crypto as crypto:
                try:
                    invoice = await crypto.create_invoice(
                        "USDT", 
                        amount, 
                        description=f"Анонимный донат",
                        payload=f"donate_{data['receiver_id']}_{int(datetime.now().timestamp())}"
                    )
                    ...
                    await message.answer(
                        ...
                        f"{invoice['pay_url']}\n\n"
                        f"<i>Ссылка действительна 30 минут</i>\n\n"
            ...
            try:
                ...
                if invoices and isinstance(invoices, list) and len(invoices) > 0:
                    ...
                    if invoice.get('status') == 'paid':
                        ...
                        # Добавляем донат анонимно (без sender_id)
                        ...
                        # Уведомляем получателя (анонимно)
                        await self.bot.send_message(
                            receiver_id,
                            f"<b>🎉 Вам зачислили анонимный донат!</b>\n\n"
                            f"💰 Сумма: {amount} USDT\n\n"
                            f"<i>Средства уже на вашем балансе. Отправитель остался анонимным.</i>"
                        )
                        
                        await callback.message.edit_text(
                            f"<b>✅ Оплата получена!</b>\n\n"
                            f"Сумма: {amount} USDT\n"
                            f"Ваш донат успешно отправлен анонимно!\n\n"
                            f"Спасибо! 🎁"
                        )
    ...
    async def process_donate_message(self, message: Message, state: FSMContext):
        ...
        # Сохраняем сообщение анонимно
        await self.db.add_message(None, receiver_id, message.text)  # sender_id = None для анонимности
        
        # Уведомляем получателя (анонимно)
        await self.bot.send_message(
            receiver_id,
            f"<b>📩 Анонимное сообщение с донатом</b>\n\n"
            f'"{message.text}"'
        )
    ...
    async def start_reply(self, callback: CallbackQuery):
        ...
        async with self.db.pool.acquire() as conn:
            msg = await conn.fetchrow("SELECT sender_id FROM messages WHERE id = $1", msg_id)
            
            if not msg or not msg['sender_id']:
                await callback.answer("❌ Нельзя ответить на это сообщение", show_alert=True)
                return
        
        state = FSMContext(self.storage, callback.from_user.id, callback.from_user.id)
        await state.update_data(reply_to_msg=msg_id, receiver_id=msg['sender_id'])
        
        await callback.message.edit_text(
            f"<b>✍️ Напишите ответ</b>\n\n"
            f"<i>Ваш ответ будет доставлен анонимно</i>",
            reply_markup=self.keyboards.cancel()
        )
    ...
    async def start_write_message(self, callback: CallbackQuery):
        ...
        await callback.message.edit_text(
            f"<b>✍️ Напишите сообщение</b>\n\n"
            f"<i>Оно будет доставлено анонимно вместе с донатом</i>",
            reply_markup=self.keyboards.cancel()
        )
    ...
    async def admin_users(self, callback: CallbackQuery):
        ...
        for user in users[:10]:
            text += (
                ...
                f"💰 {user['balance']} USDT | ✉️ {user['messages_received']}\n"
                f"📅 {user['registered_at'].strftime('%d.%m.%Y')}\n"
                f"---\n"
            )
    ...
    async def admin_transactions(self, callback: CallbackQuery):
        ...
        text = f"<b>💰 Последние донаты:</b>\n\n"
        
        for don in donations:
            text += (
                f"📥 Кому: {don['receiver_id']}\n"
                f"💵 Сумма: {don['amount']} USDT\n"
                f"📅 {don['created_at'].strftime('%d.%m.%Y %H:%M')}\n"
                f"---\n"
            )
    ...
    async def admin_checks(self, callback: CallbackQuery):
        ...
        for check in checks:
            text += (
                ...
                f"📅 {check['created_at'].strftime('%d.%m.%Y %H:%M')}\n"
                f"---\n"
            )
