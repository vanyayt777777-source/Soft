...
@dp.callback_query(lambda c: c.data.startswith('choice_') or c.data == 'cancel_game')
async def process_game_choice(callback: CallbackQuery, state: FSMContext):
    ...
    if game == "🎲 Кубик":
        ...
        msg = await callback.message.answer_dice(emoji="⚽")
        result = msg.dice.value
        # В футболе: значение 3-5 это гол? Нужно уточнить, но для примера:
    ...
    with get_db() as conn:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (telegram_id, game, bet, win_amount if win else 0, callback.data, result))
...
async def broadcast_preview(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"{message.caption or message.text or 'Медиа сообщение'}\n\n"
