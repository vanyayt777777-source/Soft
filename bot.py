...
class Database:
    ...
    async def get_transaction(self, external_id: str) -> Optional[Dict]:
        """Получение транзакции по external_id"""
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow("SELECT * FROM transactions WHERE external_id = $1", external_id)
            return dict(row) if row else None
    
    async def update_transaction_status(self, external_id: str, status: str):
        """Обновление статуса транзакции"""
        async with self.pool.acquire() as conn:
            await conn.execute("UPDATE transactions SET status = $1 WHERE external_id = $2", status, external_id)
...
def get_payment_check_keyboard(external_id: str) -> InlineKeyboardMarkup:
    """Клавиатура для проверки платежа"""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="✅ Проверить платёж", callback_data=f"check_payment_{external_id}")],
...
async def deposit_currency(callback: CallbackQuery, state: FSMContext):
    ...
    if currency == "usdt":
        await callback.message.edit_text(
            ...
            f"Введите сумму в USDT (минимум 0.01):",
            parse_mode="HTML"
        )
    else:
        await callback.message.edit_text(
            ...
            f"1 TON = 1.8 USDT\n"
            f"Введите сумму в TON (минимум 0.01):",
            parse_mode="HTML"
        )
    ...
    try:
        ...
        if pay_url:
            ...
            async with db.pool.acquire() as conn:
                ...
                """, user_id, currency.upper(), amount, display_amount, invoice_id)
            ...
            await message.answer(
                ...
                reply_markup=get_payment_check_keyboard(invoice_id),
                parse_mode="HTML"
            )
        else:
...
async def check_payment(callback: CallbackQuery, state: FSMContext):
    ...
    # Получаем транзакцию
    transaction = await db.get_transaction(invoice_id)
    ...
    if status == 'paid':
        # Платёж подтверждён
        await db.update_transaction_status(invoice_id, 'completed')
        
        # Начисляем средства
        user_id = transaction['user_id']
        amount_usdt = float(transaction['amount_usdt'])
        await db.update_balance(user_id, amount_usdt)
        
        await callback.message.edit_text(
            f"✅ <b>Платёж успешно подтверждён!</b>\n\n"
            f"💰 Сумма: {amount_usdt:.2f} USDT зачислена на ваш баланс.",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[[InlineKeyboardButton(text="🔙 В профиль", callback_data="back_to_profile")]]
            ),
            parse_mode="HTML"
        )
...
async def withdraw_menu(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        "💸 <b>Вывод средств</b>\n\n"
        "Введите сумму для вывода в USDT (минимум 2 USDT):",
        parse_mode="HTML"
    )
...
async def process_withdraw(message: Message, state: FSMContext):
    try:
        amount = float(message.text)
        if amount < 2:
            await message.answer("❌ Минимальная сумма вывода 2 USDT")
            return
...
async def show_history(callback: CallbackQuery):
    ...
    for bet in bets:
        emoji = "🎲" if bet['game_type'] == 'dice' else "🏀" if bet['game_type'] == 'basketball' else "⚽" if bet['game_type'] == 'football' else "🎯"
...
async def show_stats(callback: CallbackQuery):
    ...
    if stats:
        text += "<b>По играм:</b>\n"
        for stat in stats:
            emoji = "🎲" if stat['game_type'] == 'dice' else "🏀" if stat['game_type'] == 'basketball' else "⚽" if stat['game_type'] == 'football' else "🎯"
...
async def run_referral_bot(bot_token: str):
    ...
    try:
        ...
        @referral_dp.callback_query(F.data.startswith("check_payment_"))
        async def referral_check_payment(callback: CallbackQuery, state: FSMContext):
            invoice_id = callback.data.replace("check_payment_", "")
        ...
        # Добавляем недостающие обработчики для callback-запросов
        @referral_dp.callback_query(F.data == "back_to_main")
        async def referral_back_to_main(callback: CallbackQuery):
            await callback.message.delete()
            await callback.message.answer(
                "👋 Главное меню:",
                reply_markup=get_main_keyboard()
        ...
        @referral_dp.callback_query(F.data == "back_to_games")
        async def referral_back_to_games(callback: CallbackQuery):
            await callback.message.edit_text(
                "🎮 Выберите игру:",
                reply_markup=get_games_keyboard()
            )
