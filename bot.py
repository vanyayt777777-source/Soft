...
# Получаем токен из переменных окружения (устанавливается на хостинге)
...
# Каналы для подписки (нужно указать ID каналов!)
# Как получить ID канала: https://t.me/getidsbot
REQUIRED_CHANNELS = []  # Формат: [{"name": "Название", "id": "@channel_username"}]
    ...
    if REQUIRED_CHANNELS:
        for i in range(0, len(REQUIRED_CHANNELS), 2):
            ...
            row.append(InlineKeyboardButton(
                ...
                url=f"https://t.me/{REQUIRED_CHANNELS[i]['id'].replace('@', '')}"
            ))
            if i + 1 < len(REQUIRED_CHANNELS):
                row.append(InlineKeyboardButton(
                    text=f"📺 {REQUIRED_CHANNELS[i+1]['name']}", 
                    url=f"https://t.me/{REQUIRED_CHANNELS[i+1]['id'].replace('@', '')}"
                ))
            buttons.append(row)
    else:
...
async def create_task(creator_id: int, channel_link: str, channel_username: str, channel_name: str, reward: int, limit: int):
    ...
    async with aiosqlite.connect('bot_database.db') as db:
        ...
        # Создаем задание
        await db.execute('''
            INSERT INTO tasks (creator_id, channel_link, channel_username, channel_name, reward, total_limit)
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (creator_id, channel_link, channel_username, channel_name, reward, limit))
...
async def complete_task(user_id: int, task_id: int) -> bool:
    ...
    async with aiosqlite.connect('bot_database.db') as db:
        ...
        # Если все места заняты, деактивируем задание
        task = await get_task(task_id)
...
async def check_channel_subscription(user_id: int, channel_username: str) -> bool:
    ...
    channel_username может быть @username или числовой ID
    """
    try:
...
async def check_subscription(callback: CallbackQuery):
    ...
    if not await can_get_reward(user_id):
        await callback.answer("Ты уже получал награду сегодня! Приходи завтра.", show_alert=True)
        return
    
    # Отправляем сообщение о начале проверки
    status_msg = await callback.message.answer("🔄 Проверяю подписку...")
    
    # Проверяем каждый канал
    not_subscribed = []
    for channel in REQUIRED_CHANNELS:
        is_subscribed = await check_channel_subscription(user_id, channel['id'])
        if not is_subscribed:
            not_subscribed.append(channel['name'])
    
    if not_subscribed:
        await status_msg.edit_text(
            f"❌ *Ты не подписан на каналы:*\n" + "\n".join([f"• {name}" for name in not_subscribed]),
            parse_mode=ParseMode.MARKDOWN
        )
    else:
        # Все проверки пройдены - начисляем награду
        await update_balance(user_id, SUBSCRIBE_REWARD)
        await update_last_reward(user_id)
        ...
        await status_msg.edit_text(
            ...
            f"+{SUBSCRIBE_REWARD} монет зачислено!\n"
...
async def do_task(callback: CallbackQuery):
    ...
    await callback.message.answer(
        ...
        f"1. Перейди по ссылке и подпишись на канал\n"
        f"2. После подписки нажми кнопку ниже\n\n"
        f"*Важно:* Бот проверит подписку автоматически!",
...
async def check_task_completion(callback: CallbackQuery):
    ...
    status_msg = await callback.message.answer("🔄 Проверяю подписку на канал...")
    
    # Реальная проверка подписки через API
    is_subscribed = await check_channel_subscription(user_id, task['channel_username'])
    ...
    # Проверяем, не выполнял ли уже задание
...
async def create_task_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "⚠️ *Важно:* Канал должен быть публичным!",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def process_task_link(message: Message, state: FSMContext):
    ...
    # Проверяем, что получили username
    if not username or ' ' in username:
        await message.answer(
            "❌ Неправильная ссылка. Отправь ссылку вида:\n"
            "https://t.me/channel_name\n"
            "или\n@channel_name"
        )
        return
    
    await state.update_data(channel_link=link, channel_username=channel_username)
...
async def process_channel_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"(минимум {MIN_REWARD} монет, максимум 1000)",
        parse_mode=ParseMode.MARKDOWN
    )
...
async def process_task_reward(message: Message, state: FSMContext):
    ...
    await message.answer(
        "👥 Сколько **человек** могут выполнить это задание?\n"
...
async def process_task_limit(message: Message, state: FSMContext):
    ...
    if user['balance'] < total_cost:
        await message.answer(
            ...
            f"Выполни больше заданий или подожди новых!",
            parse_mode=ParseMode.MARKDOWN
        )
        await state.clear()
        return
...
💰 Награда исполнителю: {data['reward']} монет
👥 Лимит: {limit} человек
...
• Нажми «Я подписался» для проверки
...
💰 *ЗАРАБОТОК*
• {SUBSCRIBE_REWARD} монет за подписку на наши каналы (раз в {REWARD_COOLDOWN_HOURS}ч)
• Выполняй задания других пользователей
• Монеты можно тратить на свои задания

⚡ *ВАЖНО*
• Бот проверяет подписку автоматически
• Нельзя выполнить одно задание дважды
