...
async def quick_bet_selection(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await process_quick_bet(callback.message, state, bet_amount, callback.from_user.id)
...
async def process_mines_bet(message: Message, state: FSMContext):
    try:
        ...
        # Создаем игру
        await db.create_mines_game(user_id, game_id, bet_amount, bombs)
...
async def mines_cell_click(callback: CallbackQuery, state: FSMContext):
    ...
    game = await db.get_mines_game(game_id)
    ...
    if field[cell_index] == "bomb":
        # Проигрыш
        await db.end_mines_game(game_id, "lose")
        await callback.message.edit_text(
            f"💣 <b>Майнс</b>\n\n"
            f"💥 <b>ВЗОРВАЛОСЬ!</b>\n"
        ...
        new_multiplier = 1.0 + (len(new_opened) * 0.25)  # Множитель растет с каждым ходом
        
        await db.update_mines_game(game_id, cell_index, new_multiplier)
...
async def mines_cashout(callback: CallbackQuery, state: FSMContext):
    ...
    await db.end_mines_game(game_id, "win")
...
async def process_quick_bet(message: Message, state: FSMContext, bet_amount: float, user_id: int):
    ...
    try:
        ...
        logger.error(f"Error in process_quick_bet: {e}")
...
async def process_bet_amount(message: Message, state: FSMContext):
    try:
        ...
        if game_type == "quick":
            await process_quick_bet(message, state, bet_amount, user_id)
            return
...
async def withdraw_menu(callback: CallbackQuery):
    await callback.message.edit_text(
        "💸 <b>Вывод средств</b>\n\n"
        "Вывод только в USDT\n"
        "Введите сумму для вывода (минимум 5 USDT):",
        parse_mode="HTML"
    )
...
async def process_bot_broadcast(message: Message, state: FSMContext):
    ...
    await message.answer(f"📢 Начинаю рассылку в боте @{bot_info['bot_username']}...\nВсего пользователей: {len(users)}")
    ...
    try:
        ...
        async def referral_quick_bet(callback: CallbackQuery, state: FSMContext):
            ...
            try:
                ...
                await process_referral_quick_bet(callback.message, state, bet_amount, callback.from_user.id, bot_token)
        ...
        async def referral_process_bet(message: Message, state: FSMContext):
            try:
                ...
                if game_type == "quick":
                    await process_referral_quick_bet(message, state, bet_amount, user_id, bot_token)
                    return
        ...
        async def process_referral_quick_bet(message: Message, state: FSMContext, bet_amount: float, user_id: int, bot_token: str):
            """Обработка быстрой ставки в реферальном боте"""
            try:
                if bet_amount < 0.01:
                    await message.answer("❌ Минимальная ставка 0.01$")
                    await state.clear()
                ...
                logger.error(f"Error in process_referral_quick_bet: {e}")
                await message.answer("❌ Произошла ошибка")
                await state.clear()
        
        # Копируем обработчики Mines для реферального бота
        ...
        # Копируем обработчики пополнения и вывода для реферального бота
